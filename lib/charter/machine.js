  ...
  var Machine = function (config) {
    ...
    */
    ...
    /**
      ...
      @default true
    */
    this.errorOnUnhandledEvent = true;

    /**
      The initial state.  If not defined, the Machine will
      look for a state named 'start' to fill it.
    ...
    */
    ...
    /**
      A hash containing metadata for the machine's states.
    ...
    */
    ...
    // Initialize child states and enter initialState.
    this.init(config);
  };
  ...
  Machine.prototype = {
    ...
    */
    ...
    stateMetaFor: function (state) {
      var stateMeta = this.stateMeta[state.id];
      if (!stateMeta) {
        this.stateMeta[state.id] = stateMeta = {};
      }
      return stateMeta;
    },

    setStateMeta: function(state, key, value) {
      return this.stateMetaFor(state)[key] = value;
    },

    getStateMeta: function(state, key) {
      return this.stateMetaFor(state)[key];
    }

    /*
    Support sending multiple arguments, to be enabled later
    send: function(event) {
      var contexts = [].slice.call(arguments, 1);
      var sendRecursiveArguments = contexts;
      sendRecursiveArguments.unshift(event, this.currentState);
    ...
    sendRecursively: function(event, currentState) {
      ...
      var parentState, sendRecursiveArguments, actionArguments;
      var contexts = [].slice.call(arguments, 2);
      ...
      } else if ((parentState = currentState.parentState)) {
